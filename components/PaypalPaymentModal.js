...
class PaytmPaymentModal extends React.Component {
  ...
  _handleTransaction = data => {
    ...
    if (
      loading === false &&
      _.startsWith(url, `${ipAddress}/api/txn/paypal/status`)
    ) {
  ...
  render() {
    ...
    return (
      ...
      >
        <WebView
          source={{
            uri: `${ipAddress}/api/txn/paypal`,
            method: "post",
            body: `x-auth-token=${authToken}&ORDER_ID=${Date.now().toString()}&CUST_ID=p2@gmail.com&TXN_AMOUNT=1&CALLBACK_URL=${ipAddress}/api/txn/paypal/status`
          }}
          ref={webview => {
